...
func (u useCase) Start(allowedIPS []net.IP, outputDir string, prog []byte) error {
	...
	if err := u.eventUC.PutModeMap(uint32(0), uint32(domain.ModeIndexPrevent)); err != nil {
	...
	for {
		...
		policyStatus := u.policyCheck(allowedIPS, domainNames, event.Daddr)
...
func (u useCase) policyCheck(allowedIPS []net.IP, domainNames []string, destinationAddress uint32) string {
